  ...
  return (
    <div className="min-h-screen bg-[#f6f7f4] text-[#13202b]">
      <aside className="fixed inset-y-0 left-0 hidden w-64 flex-col border-r border-[#293b4e] bg-[#13202b] px-5 py-6 text-white lg:flex">
        ...
        <div className="mt-auto border-t border-[#293b4e] pt-4"><button type="button" onClick={() => navigate('/perfil')} className="profile-user-link w-full text-left"><p className="truncate text-sm font-semibold">{user?.nombre}</p><p className="mt-1 truncate text-xs text-[#9aaabd]">{user?.email}</p></button><button onClick={handleLogout} className="mt-4 text-xs font-semibold text-[#9fb8ff] hover:text-white">Cerrar sesión</button></div>
      </aside>
      <main className="lg:pl-64"><header className="flex items-center justify-between border-b border-[#d9e0e8] bg-[#f6f7f4] px-5 py-5 lg:px-10"><div><p className="text-sm text-[#647184]">VentasFix / {section.label}</p><h1 className="mt-2 text-2xl font-semibold tracking-tight">{section.title}</h1></div><button type="button" onClick={() => navigate('/perfil')} className="profile-header-user flex items-center gap-3 text-left" aria-label="Abrir tu perfil"><span className="hidden text-right sm:block"><span className="block text-sm font-semibold">{user?.nombre}</span><span className="block text-xs text-[#647184]">Administrador</span></span><span className="grid size-10 place-items-center rounded-full bg-[#356ae6] text-sm font-bold text-white">{user?.nombre?.charAt(0).toUpperCase()}</span></button></header><div className="p-5 lg:p-10"><Outlet /></div></main>
    </div>
  )
}
...
function App() {
  return <BrowserRouter><AuthProvider><Routes><Route path="/login" element={<LoginPage />} /><Route element={<ProtectedRoute />}><Route element={<AppShell />}><Route index element={<Navigate to="/dashboard" replace />} /><Route path="dashboard" element={<WorkspacePage />} /><Route path="usuarios" element={<WorkspacePage />} /><Route path="productos" element={<ProductosPage />} /><Route path="productos/nuevo" element={<ProductoForm />} /><Route path="productos/:id" element={<ProductoForm />} /><Route path="clientes" element={<ClientesPage />} /><Route path="clientes/nuevo" element={<ClienteForm />} /><Route path="clientes/:id" element={<ClienteForm />} /><Route path="perfil" element={<ProfilePage />} /></Route></Route><Route path="*" element={<Navigate to="/" replace />} /></Routes></AuthProvider></BrowserRouter>
}

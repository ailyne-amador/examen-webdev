...
function AppShell() {
  ...
  return (
    <div className="app-frame">
      ...
      <main className="app-main"><header className="app-header"><div><p className="app-breadcrumb">VentasFix / {section.label}</p><h1 className="app-header-title">{section.title}</h1></div><button type="button" onClick={() => navigate('/perfil')} className="header-user profile-header-user focus-ring" aria-label="Abrir tu perfil"><span className="header-user-copy hidden sm:block"><span className="header-user-name">{user?.nombre}</span><span className="header-user-role">Administrador</span></span><span className="header-avatar">{user?.nombre?.charAt(0).toUpperCase()}</span></button></header><div className="app-content"><Outlet /></div></main>
    </div>
  )
}
